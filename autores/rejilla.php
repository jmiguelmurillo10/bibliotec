...
$codusuario=$_POST["AUT_ID"];
$nombre=$_POST["AUT_NOM1"];
...
if ($bci <> "") { $where.=" AND AUT_ID='$bci'"; }
if ($bnombre <> "") { $where.=" AND AUT_NOM1 like '%".$bnombre."%'"; }
if ($bactivo <> "") { $where.=" AND AUT_BORRADO='$bactivo'"; }
...
$where.=" ORDER BY AUT_APEL1 ASC";
//$where.=" ORDER BY USR_ID DESC";


$query_busqueda="SELECT count(*) as filas FROM AUTORES WHERE AUT_BORRADO=0 AND ".$where;
...
<html>
	<head>
		...
		function ver_usuario(USR_ID) {
			parent.location.href="ver_usuario.php?AUT_ID=" + USR_ID + "&cadena_busqueda=<? echo $cadena_busqueda?>";
		}
		
		function modificar_usuario(USR_ID) {
			parent.location.href="modificar_usuario.php?AUT_ID=" + USR_ID + "&cadena_busqueda=<? echo $cadena_busqueda?>";
		}
		
		function eliminar_usuario(USR_ID) {
			parent.location.href="eliminar_usuario.php?AUT_ID=" + USR_ID + "&cadena_busqueda=<? echo $cadena_busqueda?>";
		}
				...
				if ($iniciopagina>$filas) { $iniciopagina=0; }
					if ($filas > 0) { ?>
						<? 
						   ...
						   $sel_resultado="SELECT  A.AUT_NOM1,A.AUT_APEL1 FROM AUTORES AS A WHERE A.AUT_BORRADO=0 AND ".$where;
