...
$sel_articulos="select articulos.*,familias.nombre from articulos,familias where articulos.codfamilia=familias.codfamilia and articulos.borrado=0 and articulos.stock>0 order by familias.codfamilia asc, articulos.codarticulo asc";
...
$numero_articulos=mysql_num_rows($rs_articulos);
		if ($numero_articulos>0) {		
			...
			$header=array('Item','Familia','Referencia','Descripcion','P. Compra Tienda');
			...
			while ($contador < mysql_num_rows($rs_articulos)) {
				...
				$pdf->Cell($w[4],5,number_format(mysql_result($rs_articulos,$contador,"precio_compra"),2,",","."),'LRTB',0,'C');
				$pdf->Ln();
				$item++;
				$contador++;
			}
