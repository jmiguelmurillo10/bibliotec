...
if ($accion == "alta") {
	...
	$rs_operacion =
		mysqli_query($objetoMysqli, $query_operacion);

	if ($rs_operacion) {
		$mensaje = "El usuario ha sido dada de alta correctamente";
	}
}
...
if ($accion == "modificar") {
	...
	if ($rs_query) {
		$mensaje = "Los datos del Usuario se han sido modificados correctamente";
	}
...
if ($accion == "baja") {
	...
	$rs_comprobar = mysqli_query($objetoMysqli, $query_comprobar);
...
<?
		...
		$rs_mostrar = mysqli_query($objetoMysqli, $query_mostrar);
		$codusuario = mysqli_result($rs_mostrar, 0, "USR_ID");
		$ci = mysqli_result($rs_mostrar, 0, "USR_CI");
		$nom1 = mysqli_result($rs_mostrar, 0, "USR_NOM1");
		$apel1 = mysqli_result($rs_mostrar, 0, "USR_APEL1");
	}
}
