...
if ($primero=="/") {
	...
	echo "--test de validacion sobre la creacion de copia de seguridad bdd--no se encuentra la ruta para realizar el respaldo--";
	//$DirBase="mysqldump ";
} else {
	$DirBase=$DirBase."\bin\mysqldump";
}
//echo "--".$Usuario,$Password,$BaseDeDatos;
//$executa = "$DirBase  --user=$Usuario --password=$Password   $BaseDeDatos > $archivo" ;
$executa = "$DirBase  --opt --user=$Usuario --password=$Password  $BaseDeDatos> $archivo" ;
		...
		<div id="pagina">
			<div id="zonaContenido">
				...
				<div id="botonBusqueda">
					<img src="../img/botonaceptar.jpg" onClick="aceptar()" onMouseOver="style.cursor=cursor">
			  </div>
		  </div>
		</div>
	</body>
