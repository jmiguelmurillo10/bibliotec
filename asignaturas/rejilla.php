...
$codusuario=$_POST["ASG_ID"];
$nombre=$_POST["ASG_NOMBRE"];
...
if ($bci <> "") { $where.=" AND ASG_ID='$bci'"; }
if ($bnombre <> "") { $where.=" AND ASG_NOMBRE like '%".$bnombre."%'"; }
if ($bactivo <> "") { $where.=" AND ASG_BORRADO='$bactivo'"; }
...
$where.=" ORDER BY ASG_NOMBRE ASC";
//$where.=" ORDER BY USR_ID DESC";


$query_busqueda="SELECT count(*) as filas FROM ASIGNATURA WHERE ASG_BORRADO=0 AND ".$where;
...
<html>
	<head>
		...
		function ver_usuario(USR_ID) {
			parent.location.href="ver_usuario.php?ASG_ID=" + USR_ID + "&cadena_busqueda=<? echo $cadena_busqueda?>";
		}
		
		function modificar_usuario(USR_ID) {
			parent.location.href="modificar_usuario.php?ASG_ID=" + USR_ID + "&cadena_busqueda=<? echo $cadena_busqueda?>";
		}
		
		function eliminar_usuario(USR_ID) {
			parent.location.href="eliminar_usuario.php?ASG_ID=" + USR_ID + "&cadena_busqueda=<? echo $cadena_busqueda?>";
		}
				...
				if ($iniciopagina>$filas) { $iniciopagina=0; }
					if ($filas > 0) { ?>
						<? 
						   ...
						   $sel_resultado="SELECT * FROM ASIGNATURA WHERE ASG_BORRADO=0 AND ".$where;
						...
						<tr class="<?php echo $fondolinea?>" onMouseOver="CambiaColor(this,'#000000','#ffffff')" onMouseOut="CambiaColor2(this,'#000000')" >
							...
							<td width="20%"><div align="center"><? echo mysql_result($res_resultado,$contador,"ASG_NOMBRE")?></div></td>							
